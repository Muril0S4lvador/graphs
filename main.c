...
Instancias list_files(char *base_path, Instancias instancia) {
    ...
    while ((dp = readdir(dir)) != NULL) {
        ...
        if (dp->d_type == DT_DIR) {
            ...
            instancia.average_tightness += (float)graph_return_total_demand(g) / graph_return_capacity(g);
...
int main() {
    ...
    for (size_t i = 0; i < num_dirs; ++i) {
        printf("%s: %.2f - %.2f - %.2f - %.2f\n", dirs[i], instancias[i].average_tightness, instancias[i].average_demand, instancias[i].average_nodes, instancias[i].average_trucks);
    }
